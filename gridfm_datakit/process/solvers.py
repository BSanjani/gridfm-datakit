...
def run_pf(net: Network, jl: Any, fast: Union[bool, None] = None, droop_config: Union[Dict, None] = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        droop_config: Optional dict with droop control parameters (omega_0, V_0, droop_buses, mp, mq, ref_bus).
    ...
    try:
        ...
        # Check if droop control is enabled
        if droop_config:
            # Convert NestedNamespace to dict if needed
            if hasattr(droop_config, '__dict__') and not isinstance(droop_config, dict):
                droop_dict = droop_config.__dict__
            else:
                droop_dict = droop_config
            
            if droop_dict.get('enabled', False):
                # Use our custom droop solver
                network_dict = jl.PowerModels.parse_file(temp_filename)
                
                # Prepare droop parameters
                droop_params = {
                    ...
                    'ref_bus': droop_dict.get('ref_bus', None),
                    'frequency_deadband': droop_dict.get('frequency_deadband', 0.0),
                    'voltage_deadband': droop_dict.get('voltage_deadband', 0.0),
                    'f_nominal': droop_dict.get('f_nominal', 60.0)
                }

                # Add droop config to network dictionary
                network_dict['droop_config'] = droop_params
                # Debug: check what's in droop_params
                print(f"DEBUG: droop_params = {droop_params}")
                print(f"DEBUG: droop_config in network_dict? {('droop_config' in network_dict)}")
                
                
                # Create optimizer using Julia code
                jl.seval("""
                    global droop_optimizer = optimizer_with_attributes(
                        Ipopt.Optimizer,
                        "tol" => 1e-6,
                        "print_level" => 5,
                        "max_iter" => 3000
                ...
                """)
                result = jl.solve_droop_pf(network_dict, jl.droop_optimizer)
                # result = jl.solve_droop_pf(network_dict, optimizer)
                
                if str(result["termination_status"]) != "LOCALLY_SOLVED":
                    raise RuntimeError(
                        f"Droop PF did not converge: {result['termination_status']}"
                    )
            else:
                # Droop disabled, use standard solver
            ...
            # No droop config, use standard PF solver
            result = jl.run_pf_fast(temp_filename) if fast else jl.run_pf(temp_filename)
            if (
                fast
                and str(result["termination_status"]) != "True"
