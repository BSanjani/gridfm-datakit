...
# def apply_droop_control(ppc):
#     """
#     Adjusts generator active power setpoints based on system frequency droop (P-f).
#     Calculates the steady-state frequency deviation required to balance supply/load.
#     """
#     # 1. Check if Droop is configured (we stored this in 'droop' key in Step 2)
#     if 'droop' not in ppc:
#         return ppc 
    
#     # 2. Extract Data
#     gen = ppc['gen'] # Generator matrix
#     bus = ppc['bus'] # Bus matrix
    
#     # Droop Params stored in Step 2
#     R_p = ppc['droop']['Rp']
#     f_nom = ppc['droop']['f_nom']
    
#     # 3. Calculate Mismatch
#     # Total Load (Column 2 in bus matrix is Pd)
#     total_load_pd = np.sum(bus[:, 2]) 
    
#     # Total Scheduled Gen (Column 1 in gen matrix is Pg)
#     # Note: We use the *current* setpoints (which might be the original dispatch)
#     total_gen_sched = np.sum(gen[:, 1])
    
#     # Mismatch: If Load > Gen, mismatch is positive
#     mismatch = total_load_pd - total_gen_sched
    
#     # If mismatch is tiny, skip
#     if abs(mismatch) < 1e-6:
#         return ppc

#     # 4. Calculate System Stiffness (Beta)
#     # Beta = Sum(1/R_i) . Avoid division by zero with 1e-6
#     inv_Rp = 1.0 / (R_p + 1e-6)
#     beta_sys = np.sum(inv_Rp)
    
#     # 5. Calculate Frequency Deviation (Delta f)
#     # Delta f = - Mismatch / Beta
#     delta_f = - mismatch / beta_sys
    
#     # 6. Update Generator Active Power (P)
#     # P_new = P_old - (1/R_i)*Delta_f
#     # If Freq drops (negative delta_f), P increases.
#     P_delta = - (inv_Rp * delta_f)
#     P_new = gen[:, 1] + P_delta
    
#     # 7. Enforce Limits (Pmin col 9, Pmax col 8)
#     # Note: Standard MATPOWER indices: PMAX=8, PMIN=9
#     P_min = gen[:, 9]
#     P_max = gen[:, 8]
#     P_new_clipped = np.maximum(P_min, np.minimum(P_max, P_new))
    
#     # Apply to PyPower case
#     ppc['gen'][:, 1] = P_new_clipped
    
#     # 8. Save calculated frequency (e.g., 59.95 Hz)
#     # We store it in a new key so you can save it to your dataset later if you want
#     ppc['res_freq'] = f_nom + (delta_f * f_nom)

#     return ppc


def apply_droop_control(net):
    """
    Adjusts generator active power setpoints based on system frequency droop (P-f).
    Works on the Network object directly.
    """
    # 1. Check if Droop is configured (assuming it was added to Network in Step 2)
    # We check for the 'droop' attribute or dictionary on the net object
    if not hasattr(net, 'droop') or not net.droop:
        return net 
    
    # 2. Extract Data from Network Object
    # GridFM Network objects store data in numpy arrays
    gen = net.gens
    bus = net.buses
    
    # Droop Params (accessed from the attribute we added)
    R_p = net.droop['Rp']
    f_nom = net.droop['f_nom']
    
    # 3. Calculate Mismatch
    # Total Load: Sum of active load at all buses
    # net.buses column 2 is PD (Active Power Demand)
    # Note: net.Pd might also be available, but net.buses is the source of truth for the solver
    total_load_pd = np.sum(bus[:, 2]) 
    
    # Total Scheduled Gen: Sum of active generation
    # net.gens column 1 is PG (Active Power Output)
    total_gen_sched = np.sum(gen[:, 1])
    
    # Mismatch = Load - Generation
    mismatch = total_load_pd - total_gen_sched
    ...
    # 7. Enforce Limits (PMIN col 9, PMAX col 8 for standard Matpower/GridFM idx)
    # GridFM constants: PMIN=9, PMAX=8 (imported from utils.idx_gen)
    P_min = gen[:, 9]
    P_max = gen[:, 8]
    P_new_clipped = np.maximum(P_min, np.minimum(P_max, P_new))
    
    # Apply back to Network object
    net.gens[:, 1] = P_new_clipped
    
    # 8. Save calculated frequency (optional, for dataset labels)
    # Storing it in the droop dict for reference if needed
    net.droop['res_freq'] = f_nom + (delta_f * f_nom)
...
def process_scenario_pf_mode(
    ...
    for perturbation in perturbations:
        perturbation = apply_droop_control(perturbation)
        res_dcpf = None
        if include_dc_res:
            try:
