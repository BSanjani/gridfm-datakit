...
# Load droop solver
#jl.seval("""include("pfdelta/droop_solver.jl")""")
...
def init_julia(
    ...
    # Load droop solver
        ...
        jl.seval(f'include("{droop_solver_path}")')
...
def pf_post_processing(
    ...
    if res["solution"]["pf"]:
        ...
        # it's likely a 1-based shift is needed.
        
        in_service_ids = set(str(i + 1) for i in net.idx_branches_in_service)
        
        # Filter the solution 
        solved_branches = {
            k: v for k, v in res["solution"]["branch"].items() 
            if k in in_service_ids and 'pf' in v
        }
        
        n_branches_in_service = len(net.idx_branches_in_service)
        
        assert len(solved_branches) == n_branches_in_service, (
            f"Mismatch: Filtered Solved {len(solved_branches)} vs In-Service {n_branches_in_service}. "
            f"Missing from solution: {set(in_service_ids) - set(solved_branches.keys())}"
        )
    ...
    #     n_branches_in_service = len(net.idx_branches_in_service)
    #     solved_branches = {k: v for k, v in res["solution"]["branch"].items() if 'pf' in v}
    #     # assert len(res["solution"]["branch"]) == n_branches_in_service, (
    #     #     f"Number of branches in solution ({len(res['solution']['branch'])}) "
    #     #     f"should match number of in-service branches ({n_branches_in_service})"
    #     # )
    #     assert len(solved_branches) == n_branches_in_service, (
    #         f"Mismatch: Solved {len(solved_branches)} vs In-Service {n_branches_in_service}"
    #     )
    # else:
    ...
    if "frequency_deviation" in res["solution"]:
        X_runtime[0, 2] = res["solution"]["frequency_deviation"]
    else:
        X_runtime[0, 2] = 0.0  # Default to 0 if not available

    if include_dc_res:
        if res_dc is not None:
            X_runtime[0, 3] = res_dc["solve_time"]
        else:
            X_runtime[0, 3] = np.nan
    return {
        "bus": X_bus,
        "gen": X_gen,
        "branch": X_branch,
