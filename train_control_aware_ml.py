...
INPUTS:  Load (Pd, Qd) + Control parameters (mp_droop, mq_droop)
...
data_dir = Path('./data_out_droop_case57_test/case57_ieee/raw')
...
control_params = gen_data.groupby('load_scenario_idx')[['mp_droop', 'mq_droop']].first().fillna(0.0)
...
# X = np.concatenate([X_pd, X_qd], axis=1)  # Only load, no control params



# Concatenate: [n_buses*2 load features + 3 control features]
# X = np.concatenate([X_pd, X_qd, X_mp, X_mq, X_K_I], axis=1)
X = np.concatenate([X_pd, X_qd, X_mp, X_mq], axis=1)
...
print(f"  - Control features: 2 (mp_droop, mq_droop)")
...
class ControlAwarePowerFlowNN(nn.Module):
    ...
    - Control parameters (mp_droop, mq_droop)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save({
            ...
            'val_loss': val_loss,
        }, 'best_model_droop_control_10k.pth')
...
# Load best model
checkpoint = torch.load('best_model_droop_control_10k.pth', weights_only=False)
