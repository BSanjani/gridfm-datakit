...
def correct_network(network_path: str, force: bool = False) -> str:
    ...
    try:
	    ...
	    f'data = PowerModels.parse_file("{network_path.replace(chr(92), chr(92)+chr(92))}")',
            #f'PowerModels.export_matpower("{tmp_path}", data)',
	    f'PowerModels.export_matpower("{tmp_path.replace(chr(92), "/")}", data)',
        ]
...
class Network:
    ...
    def add_droop_coefficients(ppc, config):
        """
        Injects droop control parameters into the PPC generator matrix.
        We will append extra columns to the 'gen' matrix to store Rp and Rq.
        """
        droop_cfg = config.get('droop_control', {})
        if not droop_cfg.get('enable', False):
            return ppc

        gen = ppc['gen']
        n_gen = gen.shape[0]
        
        # Define Column Indices (PyPower standard ends at col 20, so we append safely)
        # Storing them in the 'ppc' dictionary prevents losing them during processing
        if 'droop' not in ppc:
            ppc['droop'] = {}

        if droop_cfg.get('randomize_droop', True):
            rp_min, rp_max = droop_cfg['R_p_range']
            rq_min, rq_max = droop_cfg['R_q_range']
            
            # Rp: Frequency Droop (pu f / pu P)
            R_p = np.random.uniform(rp_min, rp_max, n_gen)
            # Rq: Voltage Droop (pu V / pu Q)
            R_q = np.random.uniform(rq_min, rq_max, n_gen)
        else:
            # Fixed mean values if not randomized
            R_p = np.full(n_gen, np.mean(droop_cfg['R_p_range']))
            R_q = np.full(n_gen, np.mean(droop_cfg['R_q_range']))

        # Store in the dictionary for easy access later
        ppc['droop']['Rp'] = R_p
        ppc['droop']['Rq'] = R_q
        ppc['droop']['f_nom'] = droop_cfg.get('f_nominal', 60.0)
        
        # Store initial setpoints (References) needed for the droop calculation
        # PyPower columns: 1=Pg, 5=Vg (approximate voltage setpoint)
        ppc['droop']['P_ref'] = gen[:, 1].copy()
        ppc['droop']['V_ref'] = gen[:, 5].copy()
